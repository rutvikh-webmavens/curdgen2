        ...
        <div class="col-md-12">
            @can('menu_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.menus.create') }}">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select class="search" strict="true">
                                            ...
                                            @foreach(App\Models\Menu::TYPE_SELECT as $key => $item)
                                    ...
                                    <td>
                                        <select class="search" strict="true">
                                            <option value>{{ trans('global.all') }}</option>
                                            @foreach(App\Models\Menu::ENTRIES_PER_PAGE_SELECT as $key => $item)
                                                <option value="{{ $item }}">{{ $item }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    ...
                                    <td>
                                        <select class="search" strict="true">
                                            <option value>{{ trans('global.all') }}</option>
                                            @foreach(App\Models\Menu::ORDER_BY_DESC_SELECT as $key => $item)
                                                <option value="{{ $item }}">{{ $item }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                            ...
                            <tbody>
                                @foreach($menus as $key => $menu)
                                    <tr data-entry-id="{{ $menu->id }}">
                                        ...
                                        <td>
                                            @can('menu_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.menus.show', $menu->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('menu_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.menus.edit', $menu->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('menu_delete')
                                                <form action="{{ route('frontend.menus.destroy', $menu->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('menu_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.menus.massDestroy') }}",
